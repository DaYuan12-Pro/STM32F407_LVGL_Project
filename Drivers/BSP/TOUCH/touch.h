...
// static void tp_write_byte(uint8_t data);                /* 向触摸芯片写入一个字节 */
// static uint16_t tp_read_ad(uint8_t cmd);                /* 读取AD转换值 */
// static uint16_t tp_read_xoy(uint8_t cmd);               /* 带滤波的单方向坐标读取(X/Y) */
// static void tp_read_xy(uint16_t *x, uint16_t *y);       /* 读取X、Y双方向坐标 */
// static uint8_t tp_read_xy2(uint16_t *x, uint16_t *y);   /* 带增强滤波的双方向坐标读取 */
// static void tp_draw_touch_point(uint16_t x, uint16_t y, uint16_t color);    /* 绘制触摸校准点 */
// static void tp_adjust_info_show(uint16_t xy[5][2], double px, double py);   /* 显示校准信息 */

// uint8_t tp_init(void);                 /* 初始化触摸屏 */
// static uint8_t tp_scan(uint8_t mode);  /* 扫描触摸状态 */
// void tp_adjust(void);                  /* 触摸屏校准 */
// void tp_save_adjust_data(void);        /* 保存校准参数 */
// uint8_t tp_get_adjust_data(void);      /* 读取校准参数 */
// void tp_draw_big_point(uint16_t x, uint16_t y, uint16_t color); /* 绘制大点 */
