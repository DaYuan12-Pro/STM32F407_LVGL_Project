...
// #if LV_TICK_CUSTOM
//     #define LV_TICK_CUSTOM_INCLUDE          "FreeRTOS.h"                /* 系统时间函数头文件 */
//     #define LV_TICK_CUSTOM_SYS_TIME_EXPR    (xTaskGetTickCount())       /* 获取系统当前时间的表达式（单位：毫秒） */
...
#define LV_TICK_CUSTOM 1
    ...
    /* FreeRTOS 的 tick 数转换为 LVGL 需要的毫秒数 */
    #define LV_TICK_CUSTOM_SYS_TIME_EXPR \
        ((uint32_t)(((uint64_t)xTaskGetTickCount() * 1000U) / configTICK_RATE_HZ))
#endif
